...
void decrypt(bitset<64>& bs_plain);
...
int main()
{
	...
	bitset<64> key (string("0000000100000001000000010000000100000001000000010000000100000001"));
	encrypt(plain, key);
	cout << plain << endl;
	decrypt(plain);
	cout << plain;
...
void decrypt(bitset<64>& bs_plain)
{
	...
	for (int i=15; i>=0; i--)
	{
		...
		// cout << "r(old) : " << r << endl;
		// Mixer
		mixer(r, global_round_key[i], feistel);
		// cout << "global_round_key : " << global_round_key[i] << endl;
		l ^= feistel;
		// cout << "New l : " << l << endl;
